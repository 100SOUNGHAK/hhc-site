...
| `namecard.html` | 전자명함. **QR이 정적 SVG로 박혀 있다** — 4장 참조. 링크 미리보기 og 태그 있음(한국어판) |
| `namecard-en.html` | 전자명함 **영문 og 카드용** 넘겨주기 페이지 (my-en.html 과 같은 구조) |
| `namecard-share-ko.jpg` / `namecard-share-en.jpg` | 전자명함 발송 카드에 쓰는 배너 사진 1200×600 (한/영). **파일명이 바뀌면 카카오 카드에 사진이 안 뜬다** |
...
| `schedule-widget.html` | 일정 위젯 (달력 + 오늘·내일). ☀️ 화면 켜둠(Wake Lock) 버튼 있음 · `?w=1` 초압축 모드 |
| `schedule-list-widget.html` | **일정 리스트 위젯.** 달력 없이 오늘 + 앞으로 2일만 리스트로. 위와 별개 앱 아이콘 |
...
### 기타
- `sw.js`, `my-manifest.json`, `manifest*.json`, `schedule-widget-manifest.json`,
  `schedule-list-widget-manifest.json` — PWA 설치 설정 (전부 상대경로 — 도메인 바뀌어도 손댈 필요 없음).
  매니페스트는 미러(하이픈 없는 이름) 사본이 필요 없다 — 사람이 주소를 치는 파일이 아니기 때문
- `r.html` — 단축링크 리다이렉터(`?k=`)
- `_redirects` — Cloudflare 리다이렉트. 내용은 `/  /home  301` 한 줄만. **함부로 늘리지 말 것**
  (Cloudflare는 파일이 있어도 리다이렉트를 우선하므로 잘못 넣으면 화면이 사라짐)
- `_headers` — 브라우저 캐시 규칙. `/*` 에 `Cache-Control: no-cache` 한 규칙만.
  올린 파일이 화면에 바로 반영되게 하는 장치다. `_redirects` 와 **다른 파일**이니 헷갈리지 말 것
...
7-1. **⛔ 고치기 전에 GitHub의 현재 파일과 내 사본이 같은지 반드시 확인한다.**
    작업 폴더의 사본이 오래된 것일 수 있다. 크기·수정일·`<title>` 을 GitHub 것과 대조하고,
    다르면 **GitHub 것을 받아 그 위에 고친다.**
    (2026-08-27 사고: 옛 사본에 og 태그만 얹어 올려 `namecard.html` 의 8/26 리디자인이 통째로 날아갔다.
     `git show origin/main:파일명` 또는 raw 주소로 실제 최신본을 먼저 가져올 것)
...
10. **완성 파일은 반드시 `.zip` 으로 압축해 전달한다** — 파일을 하나만 고쳤을 때도 마찬가지다.
    채팅 앱이 내려받는 파일명에서 하이픈을 지워 버려(`customer-admin.html` → `customeradmin.html`),
    사장님이 매번 손으로 이름을 되돌려야 하는 일이 생겼다. 압축 안의 이름은 앱이 건드리지 못한다.
    자세한 방법은 10장 참조.
...
### 📦 파일 전달 방식 — 반드시 압축(.zip)으로

채팅 앱은 내려받는 파일의 이름에서 **하이픈을 지운다.** 화면에 「Customer admin」처럼
보이는 것은 앱이 만든 표시용 제목이고, 실제로 저장되는 파일명에도 하이픈이 빠져 있다.
그대로 올리면 주소가 달라져 화면이 열리지 않는다.

- **파일이 1개든 10개든 `.zip` 으로 묶어서 전달한다.** 압축 안의 이름은 그대로 보존된다.
- 압축 파일 이름은 작업 내용이 드러나게 붙인다 (예: `hhc-명함발송.zip`).
- 압축 안에 **폴더를 만들지 않는다** — 파일만 평평하게 담는다.
  (폴더째 올리면 GitHub이 하위 폴더를 만들어 주소가 달라진다)
- 메시지에는 **① 안에 든 파일 목록 ② 덮어쓰기인지 새 파일인지 ③ 각 파일이 무엇을 바꾸는지**
  를 함께 적는다. 사장님이 목록과 대조할 수 있어야 한다.
- 사장님 안내: 압축을 푼 뒤 **폴더가 아니라 파일들을 선택해서** GitHub에 올린다.
  zip 파일 자체를 올리지 않는다.
...
**💼 전자명함 발송 신설**(카카오 feed 카드 = 위 사진·가운데 문구·아래 「명함보기」 버튼.
`namecard-share-ko/en.jpg` 배너 2종, 영문 미리보기용 `namecard-en.html`,
`namecard.html` 에 og 태그 추가. 카카오 카드는 제목+본문 합쳐 4줄까지만 보이므로 문구를 3줄로 유지할 것)

**예약관리** — 입력 오류 안내 개선(`******`·`---`·`미상` 등 「채워져 보이지만 번호가 아닌」 값을
콕 집어 알려주고, 확인을 누르면 접힌 카드를 펼쳐 해당 칸으로 이동 + 붉게 강조.
연락처 칸 아래 실시간 경고줄 추가)

**일정 위젯** — `schedule-widget.html` 에 ☀️ 화면 켜둠(Wake Lock) 추가 ·
**`schedule-list-widget.html` 신설**(캘린더 없이 오늘 + 앞으로 2일 리스트만.
전용 매니페스트 `schedule-list-widget-manifest.json` 로 별도 아이콘 설치)
